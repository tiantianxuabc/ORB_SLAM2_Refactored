...
public:
	...
	static Pointer Create(const std::string &strVocFile, const std::string &strSettingsFile, const eSensor sensor, const bool bUseViewer = true);
	...
	virtual void SaveTrajectoryTUM(const std::string &filename) = 0;

	// Save keyframe poses in the TUM RGB-D dataset format.
	// This method works for all sensor input.
	// Call first Shutdown()
	// See format details at: http://vision.in.tum.de/data/datasets/rgbd-dataset
	virtual void SaveKeyFrameTrajectoryTUM(const std::string &filename) = 0;

	// Save camera trajectory in the KITTI dataset format.
	// Only for stereo and RGB-D. This method does not work for monocular.
	// Call first Shutdown()
	// See format details at: http://www.cvlibs.net/datasets/kitti/eval_odometry.php
	virtual void SaveTrajectoryKITTI(const std::string &filename) = 0;
	...
	virtual std::vector<MapPoint*> GetTrackedMapPoints() = 0;
	virtual std::vector<cv::KeyPoint> GetTrackedKeyPointsUn() = 0;
};
